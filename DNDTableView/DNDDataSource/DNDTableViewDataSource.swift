...
class DNDTableViewDataSource: NSObject, NSTableViewDataSource {
    ...
    */
    ...
    func moveObjectsFromIndexes(_ indexSet: IndexSet, toIndex: Int) {
        ...
        if let selectedObjects = self.arrayController.selectedObjects {
            undoManager?.registerUndo(withTarget: self.arrayController) {ac in
                ac.setSelectedObjects(selectedObjects)
            }
        }
        ...
        while currentIndex != nil {
            ...
            if let array = self.arrayController.arrangedObjects as? Array<Any> {
                ...
                undoManager?.registerUndo(withTarget: self.arrayController) {ac in
                    if let undoArray = ac.arrangedObjects as? Array<Any> {
                        let object = undoArray[insertIndex]
